...
function RightSidebar({ sidebarOpen, setSidebarOpen }) {
  const trigger = useRef(null);
  const sidebar = useRef(null);
  const router = useRouter();
  const pathname = router.pathname;

  const storedrightSidebarExpanded = true;
  const [rightsidebarExpanded, setrightSidebarExpanded] = useState(
    storedrightSidebarExpanded === null
      ? false
      : storedrightSidebarExpanded === "true"
  );
  ...
  useEffect(() => {
    localStorage.setItem("rightsidebar-expanded", sidebarExpanded);
    if (sidebarExpanded) {
      document.querySelector("body").classList.add("rightsidebar-expanded");
    } else {
      document.querySelector("body").classList.remove("rightsidebar-expanded");
    }
  }, [sidebarExpanded]);
  ...
  return (
    <div>
      ...
      <div
        id="sidebar"
        ref={sidebar}
        className={`flex flex-col absolute z-40 right-0 top-0 lg:static lg:right-auto lg:top-auto lg:translate-x-0 h-full overflow-y-scroll lg:overflow-y-auto no-scrollbar w-64 lg:w-20 lg:rightsidebar-expanded:!w-64  shrink-0 bg-white p-4 transition-all duration-200 ease-in-out border-r border-slate-200 ${
        ...
        <div className="flex justify-start mb-10 pr-3 sm:px-2 ">
          ...
          <button onClick={() => setSidebarExpanded(!sidebarExpanded)}>
            ...
            <svg
              viewBox="0 0 24 24"
              stroke="currentColor"
              className="w-7 h-7 hidden lg:block text-slate-400 text-xl fill-current rightsidebar-expanded:rotate-180 transition-transform duration-300"
            >
              <path
                fill="currentColor"
                className="font-bold"
        ...
        <div className="space-y-8">
          ...
          <div>
            <h3 className="text-xs uppercase text-slate-500 font-semibold pl-3">
              <span
                className="hidden lg:block lg:rightsidebar-expanded:hidden  text-center w-6"
                aria-hidden="true"
              >
                •••
              </span>
              <span className="lg:hidden lg:rightsidebar-expanded:block">
                Pages
              </span>
            </h3>
            <ul className="mt-3">
              ...
              <li className="px-3 py-4 rounded-sm mb-0.5 last:mb-0">
                <Link href="/" legacyBehavior>
                  ...
                  >
                    ...
                    <span
                      className={`ml-4 text-lg transition-opacity duration-500 rightsidebar-expanded:opacity-100 rightsidebar-expanded:translate-x-0 ${
                        sidebarExpanded
